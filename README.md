...
**Current macOS admission:** `codex` and OpenCode Go. Codex relies on clone-surviving OAuth; OpenCode Go uses its fixed, one-dispatch BWS API-key bridge and does not participate in the OAuth walkthrough. Vibe/GLM-5.2 remains disabled until the native Vibe adapter is implemented and qualified. `switchyard-golden-6` is the validated Xcode-capable image.
...
currently `["codex", "opencode-go"]`). OAuth-backed providers are
added only after their clone-survival test proves the golden image's baked-in
login; OpenCode Go is admitted from its separate bounded BWS-bridge
qualification. Vibe remains disabled until its native adapter is implemented
and qualified. A provider with quota and an
