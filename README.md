...
The current package release is `0.2.0`. Check the installed dispatcher version with `switchyard-dispatch --version` (or `node src/switchyard/dispatch/index.mjs --version`); it prints the semantic version and exits successfully without requiring a project or provider configuration.
