...
**Current macOS admission:** automatic admission is descriptor-bound and must be read from `npm run roster:coherence`. On 2026-09-21 exact VM dispatch canaries passed and promoted Vibe GLM 5.3 low `zhipu/glm-5.3-medium` / `glm-5-3-medium`, Vibe standard `zhipu/glm-5.3` / `glm-5-3`, Cursor standard `grok-4.7-high`, and Cursor high `grok-4.7-xhigh`. Copilot Student also passed a current simple-path canary, and both Antigravity Gemini variants plus Antigravity Claude passed current CLI-version smoke qualification. The 2026-09-15 OpenCode Mistral GLM attempts remain separate historical failures and authorize no active descriptor. `switchyard-golden-6` is the validated Xcode-capable image.
