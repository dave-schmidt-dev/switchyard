...
**Current macOS admission:** `codex`, OpenCode Go, and Vibe/GLM-5.2. Codex and Vibe rely on clone-surviving OAuth; OpenCode Go uses its fixed, one-dispatch BWS API-key bridge and does not participate in the OAuth walkthrough. `switchyard-golden-6` is the validated Xcode-capable image.
...
Working VMs inherit the golden image's provider binaries and baked-in credential state directly through cloning — no per-VM credential step. Real full-clone dispatch canaries now cover Codex and Vibe/GLM-5.2; OpenCode Go uses its separate BWS bridge, and the remaining OAuth-backed providers require their own clone-survival proof (see "macOS queue admission and provider preflight" above).
