...
`router/health.mjs` keeps a host-owned, fenced projection of how each routing target has been behaving, and the queue consults it in one of two modes. `shadow` (the default) computes the decision, records it in the run, and changes nothing; `enforce` (`--health-enforce` or `SWITCHYARD_ROUTE_HEALTH_MODE=enforce`) also removes held targets from the candidate set with reason `route_health_suppressed`. No queue has yet been run in enforce mode, and switching a queue to it is a separate attended decision that should follow a shadow assessment over a meaningful sample of dispatches.
...
A target's state is `healthy`, `suspect`, `cooldown`, `repair-hold`, or `half-open`. Only the closed hold codes `auth_expired`, `model_unavailable`, and `cli_usage_error` create a repair hold; the transient set that would drive the 5/15/60-minute cooldown ladder is currently empty, so ordinary provider exits, timeouts, and diff-capture failures never change a target's state. A hold is keyed to the public configuration epoch (derived from the qualified-provider allowlist and `SWITCHYARD_PARALLELS_GOLDEN_IMAGE`) and to the target's repair epoch, so rebuilding the golden image or changing the allowlist invalidates holds from the old configuration. A hold clears only on an ingested `verified_transport_success` (transport, lifecycle, and served model all verified) for that target.

Leaving a hold is a half-open trial. `switchyard-dispatch health attest-repair --target <id> --repair-kind <image_repaired|auth_repaired|configuration_repaired>` records a host-side attestation; a trial is available only when an attestation is newer than the newest hold at the current repair epoch. One half-open claim per target is keyed by run id, task id, and attempt; a queue without a run identity never claims one. A claim is released only if the trial is proven never to have started; once a trial starts, the claim stays until an ingested terminal event carrying that exact claim revision resolves it, and a started trial with no lifecycle proof produces no binding and blocks the quota-fallback path for that task rather than guessing. Terminal binding requires the roster descriptor receipt to match and a transport or lifecycle proof. `health identity --target <id> --capability <low|standard|high>` prints the identity a queue would use, and `health inspect --target <id> --repair-epoch <n>` prints the projection; both are read-only.
