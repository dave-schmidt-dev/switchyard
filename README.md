...
| `src/switchyard/lifecycle/parallels-execution-backend.mjs` | Task 4.1 Parallels lifecycle: UUID-backed clone/boot/destroy, Aqua readiness polling, host-owned VM ownership records and allocation intents under the run's `resources/` directory, fresh creator-birth and current-run checks for reclamation, a bounded host-readiness probe run before a VM slot is claimed (closed codes `vm_host_inventory_permission_denied`, `vm_host_inventory_unavailable`, `vm_host_service_degraded`), an opt-in lost-mutation reconciliation gate (`enableLostMutationReconciliation`, off by default), and golden-image safety refusals. |
...
Ctrl+C is the escape from a login that will not finish. The login's own child dies of the same signal, and the walkthrough reads that and stops rather than starting the next provider's login; the golden image is stopped on the way out and the command exits 128+signo (130 for SIGINT), which is distinguishable from the exit 1 an unauthenticated provider produces.
