...
**Current macOS admission:** `codex`, OpenCode Go, and Vibe are admitted by their retained qualification evidence, but admission is not current authentication. A fresh disposable-clone check on 2026-09-14 found Vibe unauthenticated; until its setup is completed and a new clone canary passes, its historical receipts must not be read as current operability. OpenCode Go uses MiMo 2.5 for low and MiniMax M3 with `thinking` for standard; both live roster smokes passed on 2026-09-10, and production run `c578c603-f193-4e01-961e-4be2c115595b` integrated through the restored standard route with complete cleanup. DeepSeek Flash reached the provider but returned a service-side server error and was not promoted. Vibe holds two `dispatch_qualified` receipts from 2026-09-04 canaries: `low` → `zhipu/glm-5.2-low` (`sha256:bfb64bc…`) and `standard` → `zhipu/glm-5.2-high` (`sha256:4f42e879…`). Both are GLM-5.2 served through Mistral's API at different thinking levels, and each class earns its own receipt — one authorizes nothing for the other. The 2026-09-01 `mistral-medium-3.5` receipt (`sha256:0ab7279a…`) is retained as history; no slot references it, so it authorizes no route. The 2026-08-31 failure at provider execution (`exitCode` 255, output unclassifiable, no file written) was a prior attempt on this same underlying route, not on a different one — the GLM selector it wore was replaced locally by Vibe's config loader, not honoured by the provider — and its cause was never established. Its closure depended on an entitlement finding that has since been withdrawn (see above), so it is reopened rather than superseded. Enablement still is not qualification: an unpromoted target carries no evidence that its route works. OpenCode Go uses its fixed, one-dispatch BWS API-key bridge and does not participate in the OAuth walkthrough; Vibe is probeable and authenticates from the image's guest keychain when that entry is present. `switchyard-golden-6` is the validated Xcode-capable image.
