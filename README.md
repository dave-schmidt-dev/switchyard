...
| `tests/parallels-backend.test.mjs` | `ParallelsExecutionBackend` unit/live-gated tests: clone/boot/destroy, Aqua readiness polling, exact-prefix PID reclamation, golden-image safety refusals, stage-specific clone-hardening/workspace-preparation diagnostics, and host-side SDK job-misfire tolerance (bounded retry at `_call` and inside the separate bulk-transfer helper process, the provider-execution exclusion, and the classified failure metadata). |
