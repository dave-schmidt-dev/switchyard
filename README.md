...
For a Switchyard implementation task, `- **Quick checks:**` is required. Use `none` for a task with no check, or list up to four direct commands separated by `; ` (`npm run <script>`, `npm test`, or `node --test/--check <relative file>`). A clean-clone dependency install must be declared separately as `- **Quick check setup:** npm ci --ignore-scripts --offline`. Checks run on a disposable host candidate rebuilt from the exact VM task-base tree using only previously accepted task paths and admitted overlay paths; unrelated untracked files are not copied. Provider-edited check code runs under a macOS Seatbelt profile with no network access and file access limited to runtime paths and the candidate clone. A bounded passing receipt, completed integration intent, and stopped check processes are required before the queue marks the task complete. Failed and unknown receipts leave it incomplete.
