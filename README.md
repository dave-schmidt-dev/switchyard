...
**Recommended ordinary path:** `switchyard-dispatch simple` runs one provider once in a disposable local clone, checks its diff locally, and applies only declared files through the existing integration gate. Its target-aware local adapters cover Codex, Antigravity Gemini, Antigravity Claude, Copilot Student, and Vibe standard; OpenCode Go has a pinned local adapter but remains behind the no-overage funding gate; the two Antigravity targets share the `agy` harness but retain distinct owner-selected model contracts. Switchyard exists to consume eligible tier 1 and tier 2 implementation capacity before tier 3, preserving tier 3 providers for captain planning, organization, synthesis, and acceptance. Tier 1 drains one eligible provider at a time in roster order until its measured usage reaches zero; only tiers 2 and 3 balance by headroom. A selected tier 1 or tier 2 descriptor that silently disappears is a defect, not normal fallback. The command does not create a VM, run qualification, start a second orchestrator, retry a peer provider, or invoke a review chain. Existing roster enablement, active configured descriptor, capability, included subscription or quota funding, and no-overage rules remain eligibility gates; `roster.json` is unchanged.
...
**Current macOS admission:** automatic admission is descriptor-bound and must be read from `npm run roster:coherence`. On 2026-09-21 exact VM dispatch canaries passed and promoted Vibe GLM 5.3 low `zhipu/glm-5.3-medium` / `glm-5-3-medium`, Vibe standard `zhipu/glm-5.3` / `glm-5-3`, Cursor standard `grok-4.7-high`, and Cursor high `grok-4.7-xhigh`. The September 25 local simple canary found the Vibe low/medium descriptor is now rejected by the live API, so its older VM receipt does not authorize new simple admission. Copilot Student also passed a current simple-path canary, and both Antigravity Gemini variants plus Antigravity Claude passed current CLI-version smoke qualification. The 2026-09-15 OpenCode Mistral GLM attempts remain separate historical failures and authorize no active descriptor. `switchyard-golden-6` is the validated Xcode-capable image.
