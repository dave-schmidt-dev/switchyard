...
Provider output is not durable gate evidence: `boundedGateEvidence` returns no transcript. A non-empty timeout or rejected diff may be retained as a plain review file at `<checkpointPath>.partial-diffs/<taskId>.diff`; raw diff bytes are not copied into `checkpoint.json`, `events.jsonl`, or the ledger. An empty required diff therefore carries its closed diagnostic code and no provider transcript. The separate provider diagnostic artifact above is the only sanitized provider-failure evidence channel.
