...
| `tests/worker-bootstrap-fatal-metadata.test.mjs` | `writeFatalEvent`'s closed-code precedence: which diagnostic code wins when a prlctl misfire, a boot-stage error and a checkpoint-identity error are all in play, and when the resulting `exitCode`/`signal` are attached or withheld. Note the limitation stated in its own header — `worker-bootstrap.mjs` parses `process.argv` and calls `process.exit` at top level, so it cannot be imported, and this file pins the composition of the classifiers it imports against a verbatim copy of the glue rather than the glue itself. |
