...
**OpenCode Go status (2026-09-19):** the fixed BWS bridge now selects OpenCode's `build` agent with unattended approval, uses a bridge-specific failure exit code, and reconciles result cleanup without the Parallels-incompatible nested shell that previously hid failures as empty diffs. The owner-selected `opencode-go/deepseek-v4.1-flash` low and standard descriptors both reach the upstream service but currently return `Unexpected server error` and remain unqualified. No replacement descriptor has been selected automatically.
