...
The 2026-08-04 capability-reliability checkpoint adds explicit `RequiredCapability` and `Executor` placement, durable queue selection/dependency/external-blocker diagnostics, target-aware route provenance, and sanitized auth/integration failure metadata across task results, checkpoints, ledgers, events, and detached status/result. The separate Agy quota-classification/retry tranche remains parked until an approved sanitized fixture from the standing Switchyard container is available; host selector availability or remaining Gemini/Sonnet usage is not that fixture, and no live quota is spent to manufacture one.
...
| `src/switchyard/integrate/index.mjs` | Integration gate (INV-2): structural diff validation (`git apply --numstat`/`--summary`, not a content blocklist), path-escape/symlink/executable-file rejection, and explicit `allowSensitiveManifests` review for build/CI manifests. The runner accepts that opt-in only when `AllowManifests: true` and the manifest path are both declared in the task's `Files:` list. Reviewed apply is via stdin. Apply is idempotent: a non-mutating `git apply --check` runs first, and when it fails a `--reverse --check` probe treats an already-applied diff as a successful no-op (`{alreadyApplied: true}`) with no mutating apply; a genuinely conflicting (or corrupt) diff fails with git's captured stderr as the `reason` and a `reasonKind` of `corrupt_patch` vs `conflict`. The reviewed-gate contract is unchanged. |
| `src/switchyard/ledger/index.mjs` | Dispatch ledger (INV-4): JSONL append of provider/model/result per task, with the runner's default wiring now dual-writing matching records to the legacy global ledger and the project-local store-backed ledger. The legacy write remains until external dependence is ruled out and the dual-written history passes a completeness spot-check. |
...
- `~/Documents/Projects/.plans/switchyard/switchyard-capability-reliability-2026-08-04.md` — parked after the unconditional Phase 6.2 checkpoint; the quota-dependent tail remains blocked on `switchyard-agy-quota-fixture`.
- `~/Documents/Projects/.plans/switchyard/switchyard-capability-reliability-2026-08-04-tasks.md` — 22-task board for that checkpoint and its parked quota tranche.
...
npm run validate  # lint + deadcode + npm test
```
...
Each task's optional `- **RequiredCapability:**` field (`high` | `standard` | `low`) declares its INV-5 required capability up front instead of leaving it to the keyword-based `classifier.mjs` heuristic. A declared capability always wins over classification; an unrecognized value fails loud (`parseRequiredCapabilityField`/`resolveTaskRequiredCapability`, `src/switchyard/runner/index.mjs`) rather than silently falling back to a guess. Omit the field to keep the previous classify-from-description behavior.
