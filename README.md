...
| `src/switchyard/dispatch/remediate-orphaned-locks.mjs` | State-root-bound, human-confirmed CLI for project locks that automatic recovery must retain, including cleanup-failed runs with a proven dead worker and the historical cwd-derived filename shape. It resolves candidates fresh, rechecks liveness and ownership immediately before removal, and never auto-removes cleanup-failed evidence. Run `switchyard-dispatch remediate-orphaned-locks --dry-run --state-root <path>` first, then repeat without `--dry-run` for the interactive confirmation. |
