...
**Recommended ordinary path:** `switchyard-dispatch simple` runs one provider once in a disposable local clone, checks its diff locally, and applies only declared files through the existing integration gate. Tier 1 drains one eligible provider at a time in roster order until its measured usage reaches zero; only tiers 2 and 3 balance by headroom. The command does not create a VM, run qualification, start a second orchestrator, retry a peer provider, or invoke a review chain. Existing roster enablement, active configured descriptor, capability, subscription, and no-overage rules remain eligibility gates; `roster.json` is unchanged.
...
**Current macOS admission:** automatic admission is descriptor-bound and must be read from `npm run roster:coherence`. Vibe's direct-CLI GLM receipts are `low` → `zhipu/glm-5.2-low` (`sha256:bfb64…`) and `standard` → `zhipu/glm-5.2-high` (`sha256:4f42e…`); both are selected direct-Vibe descriptors. A fresh one-attempt canary per descriptor is required to refresh operational confidence after restoring the route. The 2026-09-15 OpenCode Mistral GLM attempts are retained as separate historical failures: low/high produced `empty_required_diff`; standard/max exited 255 with bounded redacted diagnostics. Neither OpenCode descriptor was promoted. `switchyard-golden-6` is the validated Xcode-capable image.
...
| `src/switchyard/simple/index.mjs` | Recommended one-task local path: one routed Codex target, one disposable local Git clone, declared-file diff enforcement, local checks, one integration, one absolute deadline, and a redacted terminal receipt. Codex uses `workspace-write`; providers needing a new host secret channel remain on the VM path. |
...
Progress goes to stderr and stdout contains one bounded JSON result. The deadline may be at most 30 minutes ahead and covers execution, checks, integration, and cleanup. A provider gets one attempt; failures name the phase and preserve a useful disposable checkout when possible. The clone shares object reads for speed but has independent Git control data, so provider Git operations cannot alter the owner repository's branches, index, or linked-worktree records. The host project is locked, pre-existing or concurrent edits to declared files are refused, undeclared changes are rejected, and provider output, prompts, commands, environment, and credential values are absent from the receipt. The current local adapter is Codex, launched ephemerally with `workspace-write` and ignored user configuration. (The tier-1 Codex Spark target that also served eligible low work here was retired by the owner on 2026-09-19.) Vibe remains on the retained VM path because its host CLI needs an API-key environment and no approved fixed BWS consumer exists for this path; Switchyard does not invent one or inherit a broad credential environment.

For an explicitly authorized task-scoped dirty baseline, add `--dirty-overlay`. Every writable path remains a repeated `--file`; every necessary read-only path is a repeated `--input` and is copied into the disposable checkout but rejected if the provider changes it. This mode transports only the exact current bytes of existing tracked regular files (up to 8 MiB each), preserves unrelated host dirt, and binds the baseline to the task, committed base, repository and host identities, declared roles, per-file hashes/sizes/modes, and a content-addressed receipt. Untracked, deleted, ignored, symlinked, non-regular, secret-shaped (including `.docker/config.json`), oversized, or drifted scoped inputs fail typed preflight before routing or provider allocation. Host `HEAD` and every scoped byte are rechecked before integration; the original baseline is the diff base, so authorized pre-existing changes are not mistaken for provider output. This does not copy the checkout, stash, or create a host commit, and it is not general dirty-worktree support.
...
The first attended real-provider acceptance passed on 2026-09-14: tier-1 `codex-spark` changed one declared fixture file, the named local check passed, integration completed once, the project lock and disposable checkout were released, and the pre/post Parallels inventories were identical. The sanitized receipt is local at `verifications/2026-09-14-simple-path-acceptance.json`. When `simple` is itself launched from an outer workspace sandbox, that outer caller must authorize access to Codex's host state and subscription authentication; the inner provider remains confined to the disposable checkout with `workspace-write` and `approval_policy="never"`.
