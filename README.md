...
Each task's optional `- **Type:**` field is `implementation` by default or `review`. Implementation tasks must declare `- **Files:**`; review tasks remain exempt while the separate review-artifact channel is designed. For implementation tasks that touch a sensitive build or CI manifest, `- **AllowManifests:** true` is required in addition to listing that manifest in `Files:`. Either condition alone is rejected by the integration gate.
