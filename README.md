...
**Current macOS admission:** `codex` and OpenCode Go are qualified. Vibe/GLM-5.2 is not qualified: the 2026-08-31 canary reached the provider and failed at provider execution (`exitCode` 255, output unclassifiable, no file written). Earlier attempts failed before provider execution for an unrelated worker-boot defect and carried no evidence about this route. Its OAuth state is not sufficient evidence that the route works. OpenCode Go uses its fixed, one-dispatch BWS API-key bridge and does not participate in the OAuth walkthrough. `switchyard-golden-6` is the validated Xcode-capable image.
...
| `src/switchyard/dispatch/disposition.mjs` | Pure, fail-closed projection of durable run evidence into bounded caller actions (`monitor`, `defer`, `recover`, `repair_contract`, `target_failed`, `complete`, or `stop`); stage-specific `clone_hardening_failed` and `workspace_prepare_failed` outcomes project to `repair_contract` and never authorize provider retry. |
...
| `tests/parallels-backend.test.mjs` | `ParallelsExecutionBackend` unit/live-gated tests: clone/boot/destroy, Aqua readiness polling, exact-prefix PID reclamation, golden-image safety refusals, and stage-specific clone-hardening/workspace-preparation diagnostics. |
...
Working VMs inherit the golden image's provider binaries and baked-in credential state directly through cloning — no per-VM credential step. Real full-clone dispatch canaries cover Codex; Vibe/GLM-5.2 has clone-surviving OAuth evidence but its route is unqualified because the one canary that reached the provider failed at provider execution. OpenCode Go uses its separate BWS bridge, and the remaining OAuth-backed providers require their own clone-survival proof (see "macOS queue admission and provider preflight" above).
