...
- `SWITCHYARD_SHARED_ACCOUNT_LEDGER`: Opt-in switch for deciding provider capacity against the subscription account rather than one project (default off). When set to `1`/`true`, a reservation's committed amount is also admitted against a shared per-account commitment store, so two checkouts cannot each believe the same subscription window is free. Audit records (reservations, fallback attempts, run store, dispatch ledger) stay project-local either way. Not yet on by default: increments 1-5 need a real observation window first.
